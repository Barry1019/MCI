...
// to store the static variables which are used
static symbols static_vars ;

// to store the local variables which are used
static symbols local_vars ;
...
ast copy_class(ast t)
{
    ...
    // reset the local variable table
    delete_variables(static_vars) ;

    return create_class(get_ann(t),myclassname,var_decs_copy,subr_decs_copy) ;
}

// eliminate the local variables which are never used
ast eliminate_static(ast copy)
{
    ...
    if ( segment == "static" )
    {
        st_variable temp = lookup_variables(static_vars, name) ;
        if ( temp.offset == -1 ) copy = create_empty() ;
    }
...
// eliminate local variables which are never used
ast eliminate_local(ast copy)
{
    string name = get_var_dec_name(copy) ;

    st_variable temp = lookup_variables(local_vars, name) ;
    if ( temp.offset == -1 ) copy = create_empty() ;
...
ast_kind check_kind(ast copy)
{
    ast statement = get_statement_statement(copy) ;
    return ast_node_kind(statement) ;
}

// eliminate redundant codes for while
ast eliminate_while(ast copy)
{
    ast statement = get_statement_statement(copy) ;
    ast condition = get_while_condition(statement) ;
    ast body = get_while_body(statement) ;
    ...
    if ( size_of_expr(condition) == 1 )
    {
        ...
        {
            bool t_or_f = get_bool_t_or_f(term) ;

            if ( !t_or_f ) copy = create_empty() ;
            ...
            if ( t_or_f )
            {
                ...
                for ( int i = 0 ; i < size ; i++ )
                {
                    ast body_inside = get_statements(body,i) ;
                    ast_kind kind = ast_node_kind(get_statement_statement(body_inside)) ;
...
// eliminate redundant codes for if
ast eliminate_if(ast copy)
{
    ast statement = get_statement_statement(copy) ;
    ast condition = get_if_condition(statement) ;
    ast if_true = get_if_if_true(statement) ;
    ...
    if ( size_of_expr(condition) == 1 )
    {
        ...
        if ( ast_node_kind(term) == ast_bool )
        {
            ...
            if ( !t_or_f ) copy = create_empty() ; else
            copy = if_true ;
        }
    }
...
// eliminate redundant codes for while
ast eliminate_if_else(ast copy)
{
    ast statement = get_statement_statement(copy) ;
    ast condition = get_if_else_condition(statement) ;
    ast if_true = get_if_else_if_true(statement) ;
    ast if_false = get_if_else_if_false(statement) ;
    bool has_return_true = false ;
    bool has_return_false = false ;
    ...
    if ( size_of_expr(condition) == 1 )
    {
        ...
        if ( ast_node_kind(term) == ast_bool )
        {
            ...
            if ( !t_or_f ) copy = if_false ; else
            copy = if_true ;
        }
    }

    int size_true = size_of_statements(if_true) ;

    for ( int i = 0 ; i < size_true ; i++ )
    {
        ast body_inside = get_statements(if_true,i) ;
        ast_kind kind = check_kind(body_inside) ;

        if ( kind == ast_return ) has_return_true = true ;
    }

    int size_false = size_of_statements(if_false) ;

    for ( int i = 0 ; i < size_false ; i++ )
    {
        ast body_inside = get_statements(if_false,i) ;
        ast_kind kind = check_kind(body_inside) ;

        if ( kind == ast_return ) has_return_false = true ;
    }

    if ( has_return_true && has_return_false ) copy = create_return() ;
...
ast copy_statements(ast t)
{
    ...
    for ( int i = 0 ; i < size ; i++ )
    {
        ...
        // if a return is found terminate all the rest
        if ( has_return ) copy = create_empty() ; else
        {
            copy = copy_statement(deci) ;
            switch (check_kind(copy))
            {
            case ast_while:
                copy = eliminate_while(copy) ;
                break ;

            case ast_if:
                copy = eliminate_if(copy) ;
                break ;

            case ast_if_else:
                copy = eliminate_if_else(copy) ;
                break ;
...
ast copy_var(ast t)
{
    ...
    // insert the variable to tables
    st_variable myvar(name, type, segment, offset) ;
    insert_variables(local_vars, name, myvar) ;
    insert_variables(static_vars, name, myvar) ;
