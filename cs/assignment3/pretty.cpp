...
static int indentation_num = 0 ;
static string indentation ;

// to tell if there are static and field variables
static bool has_static = false ;
static bool has_field = false ;
static string field_vars ;

// to increment indentation
void increment_indentation()
{
    indentation_num += 4 ;
    indentation = "" ;

    for ( int i = 0 ; i < indentation_num ; i++ )
    {
        indentation += " " ;
    }
}

// to decrement indentation
...
void walk_class_var_decs(ast t)
{
    ...
    {
        // check if it is static or field variable
        ast var = get_class_var_decs(t,i) ;
        string segment = get_var_dec_segment(var) ;

        // get right segments
        if ( segment == "static" ) has_static = true ;
        if ( segment == "local" ) segment = "var" ;
        if ( segment == "this" )
        {
            segment = "field" ;
            has_field = true ;
        }
        if ( segment == "argument" ) segment = "" ;
        
        // reposition field variables
        if ( segment == "field" )
        {
            field_vars = field_vars + indentation + segment + " " ;
            walk_var_dec(get_class_var_decs(t,i)) ;
            field_vars += " ;\n" ;
        }
        else
        {
            write_to_output(indentation) ;
    ...
    // get a new line after static if followed by field variables
    if ( has_static && has_field )
    {
        write_to_output("\n") ;
        has_static = false ;
        has_field = false ;
    ...
    if ( field_vars.length() > 0 )
    {
        write_to_output(field_vars) ;
        field_vars = "" ;
    }

    //  a new line after the last delcaration
...
void walk_var_dec(ast t)
{
    ...
    if ( segment == "this" ) field_vars = field_vars + type + " " + name ; else
...
void walk_subr_decs(ast t)
{
    ...
    for ( int i = 0 ; i < size ; i++ )
    {
        ...
        // followed by a new line except the last one
...
void walk_param_list(ast t)
{
    ...
    for ( int i = 0 ; i < ndecs ; i++ )
    {
        ...
        // add the comma after parameters
...
void walk_if_else(ast t)
{
    ...
    decrement_indentation() ;
    write_to_output(indentation + "}\n") ;
    write_to_output(indentation + "else\n") ;
    write_to_output(indentation + "{\n") ;
    increment_indentation() ;
...
void walk_expr(ast t)
{
    ...
    for ( int i = 0 ; i < term_ops ; i++ )
    {
        ...
        if ( i % 2 == 0 )
        {
            ast term_inside = get_term_term(term_op) ;
            if ( ast_node_kind(term_inside) == ast_expr ) write_to_output("(") ;
            walk_term(term_op) ;
            if ( ast_node_kind(term_inside) == ast_expr ) write_to_output(")") ;
        }
        else
...
void walk_bool(ast t)
{
    ...
    if ( _constant ) write_to_output("true") ; else
...
void walk_unary_op(ast t)
{
    ...
    {
        write_to_output(uop) ;
        ast term_term = get_term_term(term) ;
        if ( ast_node_kind(term_term) == ast_expr ) write_to_output("(") ;
        walk_term(term) ;
        if ( ast_node_kind(term_term) == ast_expr ) write_to_output(")") ;
    }
    else
...
void walk_call_as_method(ast t)
{
    ...
    {
    // do not output if it is a direct call
    case ast_this:
        break ;
    case ast_var:
        walk_var(var) ;
...
void walk_expr_list(ast t)
{
    ...
    for ( int i = 0 ; i < nexpressions ; i++ )
    {
        ...
        // add comma after parameters except the last one
