...
Customer input(string line)
{
    ...
    cus.arrival = atoi(line.substr(start + 1, end).c_str());
    start = end;
    line.replace(end, 1, "*");
    end = line.find_first_of(" ");
    cus.priority = atoi(line.substr(start + 1, end).c_str());
    cus.weight = (10 - cus.priority) * 10;
    start = end;
    line.replace(end, 1, "*");
    end = line.find_first_of(" ");
    cus.age = atoi(line.substr(start + 1, end).c_str());
    start = end;
    line.replace(end, 1, "*");
    end = line.find_first_of(" ");
    cus.ticketrequired = atoi(line.substr(start + 1, end).c_str()) * 5;
...
deque<Customer> process(deque<Customer>customers)
{
    ...
    while (!queue1.empty() || !queue2.empty() || !customers.empty() || ifStore)  //  can not stop running if there is a movement
    {
        ...
        //  move the elements to the back after new arrivals
        if (ifStore)
        {
        ...
        //  aging mechanism - promotion after new arrivals in queue1 and moving back
