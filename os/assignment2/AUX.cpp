 ...
 * @LastEditTime: 2020-10-05 15:53:09
    ...
    while ( !pages.empty() )
    {
        ...
        if ( findPage(current, frames) == -1 )
        {
            ...
            if ( frames.size() == frame_num )
            {
                ...
                // only for test
                cout << "REPLACE " << frames.front().section ;
                if (dirtyPages[frames.front().section] ) cout << "(dirty)  " ;
        ...
        {
            cout << frames[i].section << " " ;
            cout << reference_bits[frames[i].section] << " " ;
        }
        cout << endl ;
